 ...
 */
export async function isTxtExtAllowed () { return (await getSetting(Constants.ALLOW_TXT_EXTENSION_KEY)) === 'true'}

/**
 * Is the Kroki extension enabled ?
 * @returns {Promise<boolean>}
 */
export async function isKrokiEnabled () { return (await getSetting(Constants.ENABLE_KROKI_KEY)) === 'true'}

/**
 * Get the Kroki server URL.
 * @returns {Promise<String>}
 */
export async function getKrokiServerUrl () {return getSetting(Constants.KROKI_SERVER_URL_KEY)}

/**
 * Is the extension currently enabled ?
 * @returns {Promise<boolean>}
 */
export async function isExtensionEnabled () { return getSetting(Constants.ENABLE_RENDER_KEY)}

/**
 * Is "LiveReload" currently enabled on the page ?
 * @returns {Promise<boolean>}
 */
export async function isLiveReloadDetected () { return getSetting(Constants.LIVERELOADJS_DETECTED_KEY)}
...
export async function getRenderingSettings () {
  const settings = await getSettings([Constants.KROKI_SERVER_URL_KEY, Constants.ENABLE_KROKI_KEY, Constants.CUSTOM_ATTRIBUTES_KEY, Constants.SAFE_MODE_KEY, Constants.JS_KEY, Constants.JS_LOAD_KEY])
  ...
  if (customJavaScriptContent) {
    ...
    return new RenderingSettings(customAttributes, safeMode, krokiEnabled, krokiServerUrl, new CustomJavaScript(customJavaScriptContent, customJavaScriptLoadDirective))
  }
  return new RenderingSettings(customAttributes, safeMode, krokiEnabled, krokiServerUrl)
}

const getCustomScriptContent = async (customJavaScriptName) => customJavaScriptName ? getSetting(Constants.CUSTOM_JS_PREFIX + customJavaScriptName) : undefined
