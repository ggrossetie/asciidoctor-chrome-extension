import {
  appendOnce, createScriptElement,
  createStyleElement,
  createStylesheetLinkElement, decodeEntities,
  removeElement, replaceStyleElement,
...
const initializeMathJax = (eqnumsValue) => {
  appendOnce(document.head, createScriptElement({
...
const appendThemeStyle = async (themeName) => {
  ...
  if (themeNames.includes(themeName)) {
    ...
    const customThemeContent = await getSetting(Constants.CUSTOM_THEME_PREFIX + themeName)
